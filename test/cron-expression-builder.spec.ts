...
describe('CronExpressionBuilder', () => {
  ...
  describe('atMinutes()', () => {
    test('should set the minute when a valid minute is provided', () => {
      expect(cronExpBuilder.atMinutes([30]).compile()).toStrictEqual(
        '30 * * * *',
      );
    });

    test('should accept 0 as the first valid minute for cron', () => {
      expect(cronExpBuilder.atMinutes([0]).compile()).toStrictEqual(
        '0 * * * *',
      );
    });

    test('should accept 59 as the last valid minute for cron', () => {
      expect(cronExpBuilder.atMinutes([59]).compile()).toStrictEqual(
        '59 * * * *',
      );
    });
  ...
  describe('atHours()', () => {
    test('should set the hour when a valid hour is provided', () => {
      expect(cronExpBuilder.atHours([12]).compile()).toStrictEqual(
        '0 12 * * *',
      );
    });

    test('should accept 0 as the first valid hour for cron', () => {
      expect(cronExpBuilder.atHours([0]).compile()).toStrictEqual('0 0 * * *');
    });

    test('should accept 23 as the last valid hour for cron', () => {
      expect(cronExpBuilder.atHours([23]).compile()).toStrictEqual(
        '0 23 * * *',
      );
    });
  ...
  describe('every()', () => {
    ...
    test('should throw an error when an invalid unit is provided', () => {
      expect(() => cronExpBuilder.every('decade')).toThrow(
  ...
  describe('atTime()', () => {
    ...
    test('should throw an error for an invalid hour', () => {
      expect(() => cronExpBuilder.atTime('25:00')).toThrow(
        /Invalid time format/,
      );
    });

    test('should throw an error for an invalid minute', () => {
      expect(() => cronExpBuilder.atTime('23:60')).toThrow(
        /Invalid time format/,
      );
    });
    ...
    describe('should throw an error for an invalid time format', () => {
      test('1', () => {
        expect(() => cronExpBuilder.atTime('14:30:10')).toThrow(
          /Invalid time format/,
        );
      });
      ...
      test('2', () => {
        ...
        expect(() => cronExpBuilder.atTime(invalidTime)).toThrow(
          /Invalid time format/,
        );
      });
    });
    ...
    test('should overwrite existing time values', () => {
      expect(cronExpBuilder.atTime('10:00').atTime('14:30').compile()).toBe(
        '30 14 * * *',
      );
    });
  });
  ...
  describe('onWeekDays()', () => {
    test('should set a single day of the week', () => {
      expect(cronExpBuilder.onWeekDays([3]).compile()).toStrictEqual(
        '* * * * 3',
      );
    });
    ...
    test('should set multiple days of the week', () => {
      // Monday and Friday
      expect(cronExpBuilder.onWeekDays([1, 5]).compile()).toStrictEqual(
        '* * * * 1,5',
      );
    });

    test('should reject array with invalid days', () => {
      expect(() => cronExpBuilder.onWeekDays([1, 8])).toThrow(
        /Invalid day of week/,
      );
    });

    test('should reject array with negative days', () => {
      expect(() => cronExpBuilder.onWeekDays([1, -3])).toThrow(
        /Invalid day of week/,
      );
    });

    test('should reject invalid day numbers', () => {
      expect(() => cronExpBuilder.onWeekDays([8])).toThrow(
        /Invalid day of week/,
      );
    });

    test('should reject negative day numbers', () => {
      expect(() => cronExpBuilder.onWeekDays([-1])).toThrow(
        /Invalid day of week/,
      );
    });
  });
  ...
  describe('onDaysOfMonth()', () => {
    test('should set a single day of the month', () => {
      expect(cronExpBuilder.onDaysOfMonth([15]).compile()).toStrictEqual(
        '* * 15 * *',
      );
    });

    test('should set multiple days of the month', () => {
      expect(cronExpBuilder.onDaysOfMonth([1, 10, 20]).compile()).toStrictEqual(
        '* * 1,10,20 * *',
      );
    });

    test('should throw an error for invalid day of the month', () => {
      expect(() => cronExpBuilder.onDaysOfMonth([32])).toThrow(
        /Invalid day of month/,
      );
    });

    test('should not allow days less than 1', () => {
      expect(() => cronExpBuilder.onDaysOfMonth([0])).toThrow(
        /Invalid day of month/,
      );
    });
  ...
  describe('duringMonths()', () => {
    test('should set a single month', () => {
      // June
      expect(cronExpBuilder.duringMonths([6]).compile()).toStrictEqual(
        '* * * 6 *',
      );
    });
    ...
    test('should set multiple months', () => {
      // February, April, June
      expect(cronExpBuilder.duringMonths([2, 4, 6]).compile()).toStrictEqual(
        '* * * 2,4,6 *',
      );
    });
  ...
  describe('everyX()', () => {
    ...
    test('for every 3 hours', () => {
      expect(
        cronExpBuilder.everyX(3, CronTimeUnit.Hour).compile(),
      ).toStrictEqual('0 */3 * * *');
    });

    test('every 5 days of the month', () => {
      expect(
        cronExpBuilder.everyX(5, CronTimeUnit.DayOfMonth).compile(),
      ).toStrictEqual('0 0 */5 * *');
    });

    test('every 2 months', () => {
      expect(
        cronExpBuilder.everyX(2, CronTimeUnit.Month).compile(),
      ).toStrictEqual('0 0 1 */2 *');
    });
  });
});
